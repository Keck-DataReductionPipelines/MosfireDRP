...
def find_spatial_profile(inputArr, startloc, width = 2, varImage = False,
                       ...
                       extractMax = False, wavefile = None):
...
def trace_fit(inputArr,startloc = None,width=5,order=2,debug=False,nsamples=25,xlim=None):
    ...
    OUTPUT: coefficients for polyfit
    '''
    if type(inputArr) is str:
        hdulist = fits.open(inputArr)
        image = hdulist[0].data
    ...
    for i in np.arange(len(sampleLoc)):
        tempSlice = lineProfile[:,sampleLoc[i]]
        gfit = Fit.mpfitpeak(yArr,tempSlice)
        peakLoc[i] = gfit.params[1]
